       ...
       while(goMan && iter<M)
       {
        //cout <<"while2:start\n";
            ...
            if(graph->edge[iter-1].weight==graph->edge[iter].weight){
                ...
                if(tempResult){//addToTempGraph(graph->edge[iter]);
...
{//this must implements union-find
    ...
    else { // we have found a useless edge
        useless++;
...
{
    //cout <<"Find: pass0\n";
    if(i>M) return (-1);//we may have no more edges!!!  Is that right???? Maybe not!!!
